...
namespace pgn
    ...
    static const char token_string [] [12] =
        {
            "Symbol", "Integer", "String", "NAG", "Asterisk", "Period", "LParen", "RParen", "LBracket", "RBracket",
        "LT", "GT", "Invalid", "None"
        };
    ...
    class UnexpectedToken
        ...
        UnexpectedToken (Token expected, const char * found, size_t line)
            {
            sprintf(buf, "Expected <%s> but found '%s', line=%llu", token_string[expected], found, line);
            }
    ...
    class PGNFileReader
        ...
        bool startOfElement ()
            {
            return startOfMoveNumberIndication() || startOfSANMove() || startOfNumericAnnotationGlyph();
            }
        ...
        bool startOfSANMove ()
            {
            return token_ == Symbol
                && (startOfPawnMove(token_str) || startOfCastleMove(token_str) || startOfMove(token_str));
            }
        ...
        bool startOfCapture (const char * p)
            {
            return (strlen(p) && p[0] == 'x') || (strlen(p) > 1 && p[1] == 'x' && isRankDigit(p, from_rank_))
        ...
        bool startOfQuietMove (const char * p)
            {
            return (strlen(p) > 1 && isSquare(p, from_square_)) || (strlen(p) && isRankDigit(p, from_rank_))
        ...
        bool readSymbol ()
            ...
            for (; ; )
                ...
                if (! isAlNum(ch_) && ch_ != '_' && ch_ != '+' && ch_ != '/' && ch_ != '#' && ch_ != '=' && ch_ != ':'
                    && ch_ != '-')
                    {
                    break;
                    }
