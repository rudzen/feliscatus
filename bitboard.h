namespace bitboard
    ...
    __forceinline const uint64_t & bbSquare (int sq)
        {
        return bb_square[sq];
        }

    __forceinline const uint64_t & bbRank (int rank)
        {
        return bb_rank[rank];
        }

    __forceinline const uint64_t & bbFile (int sq)
        {
        return bb_file[sq];
        }

    __forceinline uint64_t northOne (const uint64_t & bb)
        {
        return bb << 8;
        }

    __forceinline uint64_t southOne (const uint64_t & bb)
        {
        return bb >> 8;
        }

    __forceinline uint64_t eastOne (const uint64_t & bb)
        {
        return (bb & ~HFILE) << 1;
        }

    __forceinline uint64_t westOne (const uint64_t & bb)
        {
        return (bb & ~AFILE) >> 1;
        }

    __forceinline uint64_t northEastOne (const uint64_t & bb)
        {
        return (bb & ~HFILE) << 9;
        }

    __forceinline uint64_t southEastOne (const uint64_t & bb)
        {
        return (bb & ~HFILE) >> 7;
        }

    __forceinline uint64_t southWestOne (const uint64_t & bb)
        {
        return (bb & ~AFILE) >> 9;
        }

    __forceinline uint64_t northWestOne (const uint64_t & bb)
        {
        return (bb & ~AFILE) << 7;
        }

    __forceinline uint64_t northFill (const uint64_t & bb)
        {
        uint64_t fill = bb;
        fill |= (fill << 8);
        fill |= (fill << 16);
    ...
    __forceinline uint64_t southFill (const uint64_t & bb)
        {
        uint64_t fill = bb;
        fill |= (fill >> 8);
        fill |= (fill >> 16);
    ...
    void initBetweenBitboards (const uint64_t from, uint64_t (* stepFunc) (const uint64_t &), int step)
        {
    ...
    void init ()
        ...
        for ( uint64_t sq = a1; sq <= h8; sq++ )
            ...
            passed_pawn_front_span[0][sq] =
                pawn_east_attack_span[0][sq] | pawn_front_span[0][sq] | pawn_west_attack_span[0][sq];
            passed_pawn_front_span[1][sq] =
                pawn_east_attack_span[1][sq] | pawn_front_span[1][sq] | pawn_west_attack_span[1][sq];
    ...
    const uint64_t rank_1[2] =
        {
        RANK1, RANK8
        };

    const uint64_t rank_3[2] =
        {
        RANK3, RANK6
        };

    const uint64_t rank_7[2] =
        {
        RANK7, RANK2
        };

    const uint64_t rank_6_and_7[2] =
        {
        RANK6 | RANK7, RANK2 | RANK3
        };

    const uint64_t rank_7_and_8[2] =
        {
