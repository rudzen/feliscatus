...
namespace Keys
{
  ...
  consteval std::array<std::uint64_t, Length> snip()
  {
#ifdef _MSC_VER
#define U64(u) (u##ui64)
#else
#define U64(u) (u##ULL)
    ...
    static_assert(Offset <= 781);
    static_assert(Offset + Length <= 781);
    ...
    */
    ...
    constexpr std::array<std::uint64_t, 781> arr = {
    ...
    std::array<std::uint64_t, Length> result{};

    std::copy(std::next(arr.begin(), Offset), std::next(arr.begin(), Offset + Length), result.begin());

    return result;
  }

  constexpr std::array<std::array<std::uint64_t, SQ_NB>, PIECE_NB> poly_pc_keys = {
    snip<64 * 1, 64>(),    // poly:  1, Piece: W_PAWN
    snip<64 * 3, 64>(),    // poly:  3, Piece: W_KNIGHT
    snip<64 * 5, 64>(),    // poly:  5, Piece: W_BISHOP
    snip<64 * 7, 64>(),    // poly:  7, Piece: W_ROOK
    snip<64 * 9, 64>(),    // poly:  9, Piece: W_QUEEN
    snip<64 * 11, 64>(),   // poly: 11, Piece: W_KING
    {0},
    {0},
    snip<64 * 0, 64>(),    // poly:  0, Piece: B_PAWN
    snip<64 * 2, 64>(),    // poly:  2, Piece: B_KNIGHT
    snip<64 * 4, 64>(),    // poly:  4, Piece: B_BISHOP
    snip<64 * 6, 64>(),    // poly:  6, Piece: B_ROOK
    snip<64 * 8, 64>(),    // poly:  8, Piece: B_QUEEN
    snip<64 * 10, 64>(),   // poly: 10, Piece: B_KING
    {0},
    {0}};

  constexpr std::array<std::uint64_t, FILE_NB> poly_en_passant_keys = snip<772, FILE_NB>();

  constexpr std::array<std::uint64_t, 4> poly_castle_keys = snip<768, 4>();

  constexpr std::uint64_t poly_side_key = snip<780, 1>()[0];

  template<PieceT P, SquareT S>
  [[nodiscard]]
  constexpr std::uint64_t pc_key(const P pc, const S sq)
  {
    return poly_pc_keys[pc][sq];
  }

  template<FileT F>
  [[nodiscard]]
  constexpr std::uint64_t en_passant_key(const F f)
  {
    return poly_castle_keys[f];
  }

  [[nodiscard]]
  constexpr std::uint64_t castle_key(const std::ptrdiff_t idx)
  {
    return poly_castle_keys[idx];
  }

  [[nodiscard]]
  constexpr std::uint64_t side_key()
  {
    return poly_side_key;
  }
