...
public:
  ...
  virtual void postInfo(const int depth, int selective_depth, uint64_t node_count, uint64_t nodes_per_sec, uint64_t time, int hash_full) = 0;
...
public:
  ...
  int InputAvailable() {
    ...
    if (!console)
    {
      ...
      if (!PeekNamedPipe(hInput, 0, 0, 0, &total_bytes_avail, 0))
        return true;
  ...
  virtual void postInfo(const int depth, int selective_depth, uint64_t node_count, uint64_t nodes_per_sec, uint64_t time, int hash_full) {
  ...
  virtual void postPV(const int depth, int max_ply, uint64_t node_count, uint64_t nodes_per_second, uint64_t time, int hash_full, int score, const char *pv, int node_type) {
    char bound[24];
  ...
  bool handleSetOption(const char *params[], int num_params) {
    ...
    if (parseOptionName(param, params, num_params, &option_name) && parseOptionValue(param, params, num_params, &option_value))
    {
