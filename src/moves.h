...
class Moves {
public:
  void generateMoves(MoveSorter *sorter = 0, const uint32_t transp_move = 0, const int flags = 0) {
  ...
  void generateCapturesAndPromotions(MoveSorter *sorter) {
    reset(sorter, 0, QUEENPROMOTION | STAGES);
    max_stage = 2;
    stage     = 1;
  }

  void generateMoves(const int piece, const uint64_t &to_squares) {
    reset(0, 0, 0);
  ...
  void generatePawnMoves(bool capture, const uint64_t &to_squares) {
    reset(0, 0, 0);
  ...
  int moveCount() { return number_moves; }

  void gotoMove(int pos) { iteration = pos; }

  bool isPseudoLegal(const uint32_t m) {
...
private:
  ...
  void addMove(const int piece, const uint64_t from, const uint64_t to, const uint32_t type, const int promoted = 0) {
    uint32_t move;
    int captured;
  ...
  void addMoves(const uint64_t &to_squares) {
    uint64_t bb;
  ...
  void addPawnQuietMoves(const uint64_t &to_squares) {
  ...
  void addPawnCaptureMoves(const uint64_t &to_squares) {
    const uint64_t &pawns = board->pawns(side_to_move);
  ...
  void addCastleMove(const uint64_t from, const uint64_t to) { addMove(King | (side_to_move << 3), from, to, CASTLE); }

  bool givesCheck(const uint32_t m) {
    board->makeMove(m);
  ...
  bool isLegal(const uint32_t m, const int piece, const uint64_t from, uint32_t type) {
  ...
  bool canCastleShort() { return (castle_rights & oo_allowed_mask[side_to_move]) && isCastleAllowed(oo_king_to[side_to_move], side_to_move); }

  bool canCastleLong() { return (castle_rights & ooo_allowed_mask[side_to_move]) && isCastleAllowed(ooo_king_to[side_to_move], side_to_move); }

  bool isCastleAllowed(uint64_t to, const int side_to_move) {
    ...
    uint64_t bb_castle_span = bb_castle_pieces | bbSquare(rook_to) | bbSquare(to) | bb_between[king_square][rook_from] | bb_between[rook_from][rook_to];
