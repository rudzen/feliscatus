...
i32 KRBKX(Material* m, const i32 eval, const u32 key2) {
  switch (key2 & ~all_pawns) {
    case kr: m->drawish = 16; break;

    case kbb:
    case kbn:
    case knn: m->drawish = 8; break;
...
i32 KRNKX(Material* m, const i32 eval, const u32 key2) {
  switch (key2 & ~all_pawns) {
    case kr: m->drawish = 32; break;

    case kbb:
    case kbn:
    case knn: m->drawish = 16; break;
...
i32 KBNKX(Material* m, const i32 eval, const u32 key2, const i32 pc1, const i32 pc2, const Color c1) {
  switch (key2 & ~all_pawns) {
    ...
    case kb: m->drawish = 8; break;

    case kn: m->drawish = 4; break;
...
i32 KBxKX(Material* m, const i32 eval, const u32 key1, const u32 key2, const Color c1) {
  switch (key2 & ~all_pawns) {
    ...
    case k: return KBxKx(m, eval, key1, key2, c1);
...
i32 evaluate(Material* m, i32& flags, const int eval, const Board* b, const Color us) {
  ...
  switch (strong_key & ~all_pawns) {
    case kqb: score = KQBKX(m, score, weak_key); break;

    case kqn: score = KQNKX(m, score, weak_key); break;

    case krb: score = KRBKX(m, score, weak_key); break;

    case krn: score = KRNKX(m, score, weak_key); break;

    case kr: score = KRKX(m, score, weak_key); break;

    case kbb: score = KBBKX(m, score, weak_key); break;

    case kbn: score = KBNKX(m, score, weak_key, strong_pawn_count, weak_pawn_count, strong_side); break;

    case kb: score = KBKX(m, score, strong_key, weak_key, strong_pawn_count, weak_pawn_count, strong_side, weak_side, us); break;

    case kn: score = KNKX(m, score, weak_key, strong_pawn_count, weak_pawn_count, strong_side, weak_side, us); break;

    case knn: score = KNNKX(m, score, weak_key, strong_pawn_count); break;

    case k: score = KKx(m, score, strong_pawn_count, weak_pawn_count, strong_side); break;
