...
enum Color : e8 { WHITE, BLACK, COL_NB };
...
enum NodeType : e8 { NO_NT = 0, EXACT = 1, BETA = 2, ALPHA = 4 };

enum File : e8 { FILE_A, FILE_B, FILE_C, FILE_D, FILE_E, FILE_F, FILE_G, FILE_H, FILE_NB };
...
enum Rank : e8 { RANK_1, RANK_2, RANK_3, RANK_4, RANK_5, RANK_6, RANK_7, RANK_8, RANK_NB };
...
enum PieceType : e8 { PAWN = 0, KNIGHT = 1, BISHOP = 2, ROOK = 3, QUEEN = 4, KING = 5, NO_PT = 6, ALL_PIECE_TYPES = 6, PIECETYPE_NB = 8 };
...
constexpr std::array<Piece, PieceTypes.size() * 2> Pieces{W_PAWN, W_KNIGHT, W_BISHOP, W_ROOK, W_QUEEN, W_KING, B_PAWN, B_KNIGHT, B_BISHOP, B_ROOK, B_QUEEN, B_KING};
...
enum Move : std::uint32_t { MOVE_NONE = 0 };

template<typename T>
concept MoveT = std::is_convertible_v<T, Move>;

enum MoveType : e8 { NORMAL = 0, DOUBLEPUSH = 1, CASTLE = 1 << 1, EPCAPTURE = 1 << 2, PROMOTION = 1 << 3, CAPTURE = 1 << 4 };
...
enum MoveGenFlags { NONE = 0, LEGALMOVES = 1, STAGES = 1 << 1, CAPTURES = 1 << 2, QUIET = 1 << 3 };
...
enum MoveStage : e8 { TT_STAGE, CAPTURE_STAGE, QUIET_STAGE, END_STAGE };

template<typename T>
concept Okayable = PieceT<T> || SquareT<T> || PieceTypeT<T> || MoveT<T>;
...
[[nodiscard]]
constexpr Move init_move(const P pc, const P cap, const S from, const S to, const P promoted) {
  return static_cast<Move>(pc << 26 | cap << 22 | promoted << 18 | Mt << 12 | from << 6 | static_cast<i32>(to));
}

template<PieceT P, SquareT S>
[[nodiscard]]
constexpr Move init_move(const P pc, const P captured, const S from, const S to, const MoveType mt, const P promoted) {
  return static_cast<Move>(pc << 26 | captured << 22 | promoted << 18 | mt << 12 | from << 6 | static_cast<i32>(to));
}
