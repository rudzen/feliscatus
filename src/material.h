...
class Material
    ...
    Material ()
        {
        max_value_without_pawns =
            2 * (2 * piece_value[Knight] + 2 * piece_value[Bishop] + 2 * piece_value[Rook] + piece_value[Queen]);
    ...
    __forceinline void remove (const int p)
        {
        updateKey(p >> 3, p & 7, - 1);
        material_value[p >> 3] -= piece_value[p & 7];
        }

    __forceinline void add (const int p)
        {
        updateKey(p >> 3, p & 7, 1);
        material_value[p >> 3] += piece_value[p & 7];
        }
    ...
    __forceinline int count (const int c, const int p)
        {
        return (key[c] >> piece_bit_shift[p]) & 15;
        }

    __forceinline void makeMove (const uint32_t m)
        {
        if (isCapture(m))
            {
    ...
    __forceinline bool isKx (const int c)
        {
    ...
    __forceinline int value (const int c)
        {
    ...
    __forceinline int KBNK (int eval, int side1)
        ...
        return eval + 175 - min(25 * dist[a_winning_cornersq][loosing_kingsq],
            25 * dist[another_winning_cornersq][loosing_kingsq]);
        }

    __forceinline int KBKX (int eval, uint32_t key1, uint32_t key2, int pc1, int pc2, const int side1,
        const int side2, int side_to_move)
        {
        if (pc1 > 0)
            {
            ...
            case k:
                ...
                if (pc1 + pc2 == 0)
                    ...
                    if (side1 == side_to_move || ! board->isAttacked(lsb(board->bishops(side1)), side2))
                        {
                        if (pawn_front_span[side2][lsb(board->pawns(side2))] & (bishopAttacks(lsb(bishopbb),
                            board->occupied) | bishopbb))
                            {
                            return drawScore();
                            }
                        }
    ...
    __forceinline int KNKX (int eval, uint32_t key2, int pc1, int pc2, const int side1, const int side2,
        int side_to_move)
        {
        switch (key2 & ~ all_pawns)
            {
            case k:
                ...
                if (pc1 == 0 && pc2 == 1)
                    ...
                    if (side1 == side_to_move || ! board->isAttacked(lsb(board->knights(side1)), side2))
                        {
                        if (pawn_front_span[side2][lsb(board->pawns(side2))] & (knightAttacks(lsb(knightbb))
                            | knightbb))
                            {
                            return drawScore();
                            }
    ...
    __forceinline int KBpK (int eval, int side1)
        ...
        if (! sameColor(promosq1, lsb(board->bishops(side1))))
            ...
            if ((promosq1 == h8 && (bbk2 & corner_h8)) || (promosq1 == a8 && (bbk2 & corner_a8))
...
int Material::piece_bit_shift[7] =
    {
    0, 4, 8, 12, 16, 20
    };

int Material::piece_value[6] =
    {
    100, 400, 400, 600, 1200, 0
    };
