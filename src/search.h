class Search :
    ...
    Search (Protocol * protocol, Game * game, Eval * eval, See * see, HashTable * transt)
        {
        init(protocol, game, eval, see, transt);
        }
        ...
        while (const auto move_data = pos->nextMove())
            ...
            if (makeMoveAndEvaluate(move_data->move, alpha, beta))
                ...
                if (move_count == 1 && pv)
                    ...
                    {
                    auto next_depth =
                        nextDepthNotPV(pv, depth, move_count, move_data, alpha, best_score, expectedNodeType);
    ...
    __forceinline bool shouldTryNullMove (const int depth, const int beta) const
        {
        return ! pos->in_check && pos->null_moves_in_row < 1 && ! pos->material.isKx(pos->side_to_move)
            && pos->eval_score >= beta;
        }

    __forceinline int nextDepthNotPV (bool pv, const int depth, const int move_count, const MoveData * move_data,
        const int alpha, int & best_score, int expectedNodeType)
        {
    ...
    __forceinline int nullMoveReduction (const int depth) const
        {
        return 4 + depth / 4;
        }
        ...
        if (ply == 0)
            ...
            if (protocol && verbosity > 0)
                ...
                protocol->postPV(search_depth, max_ply, node_count * num_workers_, nodesPerSecond(),
                    max(1ull, start_time.millisElapsed()), transt->getLoad(), score, buf, node_type);
                }
            }
        }
    ...
    __forceinline bool isKillerMove (const uint32_t m, int ply) const
        {
    ...
    __forceinline int searchNodeScore (const int score) const
        {
        return score;
        }

    __forceinline int storeSearchNodeScore (const int score, const int depth, const int node_type,
        const uint32_t move)
        {
    ...
    __forceinline int drawScore () const
        {
    ...
    __forceinline bool isHashScoreValid (const int depth, const int alpha, const int beta)
        ...
        return pos->transposition && pos->transp_depth >= depth
            && ((pos->transp_type & EXACT) || ((pos->transp_type & BETA) && pos->transp_score >= beta)
                || ((pos->transp_type & ALPHA) && pos->transp_score <= alpha));
        }

    __forceinline int nodeType (const int score, const int beta, const uint32_t move) const
        {
    ...
    __forceinline bool isPassedPawnMove (const uint32_t m) const
        {
...
int Search::futility_margin[4] =
    {
    150, 150, 150, 400
    };

int Search::razor_margin[4] =
    {
    0, 125, 125, 400
    };