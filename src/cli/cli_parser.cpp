...
const ParserSettings& CliParser::parse() {
  ...
  if constexpr (T == Tuner) {
    auto* const fileOption   = m_app.add_option("-f,--file", m_parser_settings.file_name, "The PGN file to read as input");
    ...
    auto* const coordOption  = m_app.add_flag("--coordination", m_parser_settings.coordination, "Enables bishop pair tuning");
    auto* const strOption    = m_app.add_flag("--strength", m_parser_settings.strength, "Enable attack strength evaluation tuning");
    auto* const weakOption   = m_app.add_flag("--weakness", m_parser_settings.weakness, "Enable weakness evaluation tuning");
    ...
    auto* const lazyOption   = m_app.add_flag("--lazy_margin", m_parser_settings.lazy_margin, "Enables lazy_margin cut-off tuning");
...
std::unique_ptr<ParserSettings> makeParser(const int argc, char** argv, const std::string& title, const ParserType type) {
  ...
  if (type == Tuner) return std::make_unique<ParserSettings>(CliParser(argc, argv, title).parse<Tuner>());

  if (type == Engine) return std::make_unique<ParserSettings>(CliParser(argc, argv, title).parse<Engine>());
